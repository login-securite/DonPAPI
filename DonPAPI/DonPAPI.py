...
def main():
    ...
    parser.add_argument('target', nargs='?', action='store',
                        help='[[domain/]username[:password]@]<targetName or address>', default='')
    parser.add_argument('-credz', action='store',
                        help='File containing multiple user:password or user:hash for masterkeys decryption')
    ...
    group.add_argument('-H', '--hashes', action="store", metavar="LMHASH:NTHASH",
    ...
    group.add_argument('-aesKey', action="store", metavar="hex key",
                       help='AES key to use for Kerberos Authentication (1128 or 256 bits)')
    ...
    group.add_argument('--no_remoteops', action="store_true", help='do not hunt for SAM and LSA with remoteops',
                       default=False)
    group.add_argument('--GetHashes', action="store_true", help="Get all users Masterkey's hash & DCC2 hash",
                       default=False)
    group.add_argument('--no_recent', action="store_true", help="Do not hunt for recent files", default=False)
    group.add_argument('--no_sysadmins', action="store_true",
                       help="Do not hunt for sysadmins stuff (mRemoteNG, vnc, keepass, lastpass ...)", default=False)
    group.add_argument('--from_file', action='store',
                       help='Give me the export of ADSyncQuery.exe ADSync.mdf to decrypt ADConnect password',
                       default='adsync_export')
    ...
    options.domain, options.username, options.password, options.address = re.compile(
...
def load_configs(options):
    ...
    _config_data = resource_string(__name__, 'config/' + _config_file)
    config_parser = json.loads(_config_data.decode())
